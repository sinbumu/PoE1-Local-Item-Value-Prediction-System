...
import type { TrainingFeatureLabeled } from "../types/training-features.types";

const TRAINING_FEATURE_LABELED_COLUMNS = [
  "listing_key",
  "source_updated_at",
  "league",
  "model_segment",
...
function buildRowValues(feature: TrainingFeatureLabeled): Array<unknown> {
  return [
    ...
    feature.gemTags,
    feature.exchangeRateSource,
    feature.exchangeRateSampleTimeUtc,
    feature.exchangeRateChaosEquivalent,
    feature.targetPriceChaos,
    feature.targetPriceLog1p,
    feature.labelReason,
    new Date().toISOString(),
  ];
}

export class TrainingFeatureLabeledRepository {
  ...
  async upsertMany(features: TrainingFeatureLabeled[]): Promise<number> {
  ...
  async deleteOlderThan(sourceUpdatedAt: string): Promise<number> {
    const result = await pool.query<{ deleted_count: string }>(
      `
        WITH deleted AS (
          DELETE FROM training_features_labeled
