...
import type { TrainingFeatureClean } from "../types/training-features.types";

const TRAINING_FEATURE_CLEAN_COLUMNS = [
  "listing_key",
  "source_updated_at",
  "league",
  "model_segment",
...
function buildRowValues(feature: TrainingFeatureClean): Array<unknown> {
  return [
    feature.listingKey,
    feature.sourceUpdatedAt,
    feature.league,
    feature.modelSegment,
    feature.cleanReason,
    feature.targetPriceAmount,
    feature.targetPriceCurrency,
    feature.itemClass,
    feature.baseType,
    feature.rarity,
    feature.frameType,
...
export class TrainingFeatureCleanRepository {
  ...
  async upsertMany(features: TrainingFeatureClean[]): Promise<number> {
    ...
    const chunkSize = Math.max(
      1,
      Math.floor(MAX_BIND_PARAMETERS / TRAINING_FEATURE_CLEAN_COLUMNS.length),
    );
    ...
    for (let offset = 0; offset < features.length; offset += chunkSize) {
      ...
      const placeholders = chunk
        .map((_feature, rowIndex) => {
          const start = rowIndex * TRAINING_FEATURE_CLEAN_COLUMNS.length;
          const rowPlaceholders = TRAINING_FEATURE_CLEAN_COLUMNS.map(
      ...
      await pool.query(
        `
          INSERT INTO training_features_clean (
            ${TRAINING_FEATURE_CLEAN_COLUMNS.join(", ")}
          )
          VALUES ${placeholders}
