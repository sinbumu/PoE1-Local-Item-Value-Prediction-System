...
const TRAINING_FEATURE_CLEAN_CURSOR_KEY = "training_features_clean_cursor_v1";

export class TrainingFeatureCleanStateRepository {
  async getCursor(): Promise<TrainingFeatureCursor | null> {
    const result = await pool.query<{ state_value: string }>(
      `
        ...
        WHERE state_key = $1
      `,
      [TRAINING_FEATURE_CLEAN_CURSOR_KEY],
    );
    ...
    await pool.query(
      `
        ...
        DO UPDATE SET
          ...
          updated_at = NOW()
      `,
      [TRAINING_FEATURE_CLEAN_CURSOR_KEY, JSON.stringify(cursor)],
    );
  }
