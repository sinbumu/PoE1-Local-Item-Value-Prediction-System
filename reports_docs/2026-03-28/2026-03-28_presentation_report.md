...
관측 유지 시간 분포를 보면, 한 번 관측된 매물이 짧은 시간 안에 다시 보이지 않게 되는 경우만 있는 것이 아니라 일정 시간 이상 반복 관측되는 경우도 큰 비중을 차지합니다. 이는 향후 stale listing 정리 기준과 학습용 데이터 유지 기간을 정하는 데 중요한 관찰 결과입니다.
